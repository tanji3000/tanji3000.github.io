...
상황) 아래의 데이터가 있다고 가정해 봅시다.   
- Kim의 English 점수에 결측치가 있네요. 이렇게 결측치가 
...
```
파일이름 `example_R_NA.csv`를 `"(큰따옴표)`로 묶어줍니다. header=T는 id, name, Math, English, Physics가 있는 헤터를 불러와서 Variable 이름으로 만들어 줍니다.
